...
class TestChannelState(unittest.TestCase):
    ...
    def test_handle_message_timer_running(self):
        ...
        self.channel_state.bot.process_message.assert_called_once_with(
            "user", "message")
    ...
    def test_response_timer_sending(self):
        ...
        self.channel_state.start_rate_limit_timer()
        time.sleep(0.5)
        self.channel_state.message_sender.send.assert_called_once_with(
            "response")
        self.assertAlmostEqual(
            self.channel_state.last_message_time, time.time(), delta=0.7)
    ...
    def test_send_response(self):
        ...
        self.channel_state.message_sender.send.assert_called_once_with(
            "response")
        self.assertAlmostEqual(
            self.channel_state.last_message_time, time.time(), delta=0.1)
