...
# Heroku uses the old postgres:// scheme, but SQLAlchemy requires the new postgresql:// scheme
...
def double_crux(client, ack, say, command):
    ...
    say(
        f"Starting a new double crux session between {participant_a} and {participant_b}...")
    ...
    if DEBUG_MODE and len(params) > 2:
        ...
        try:
            ...
            channel_state.bot = Facilitator(
                participant_a, participant_b, custom_instructions)
