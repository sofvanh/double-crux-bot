...
class Facilitator:
  ...
  e4_bot = langchain.chat_models.ChatOpenAI(model_name="gpt-4-1106-preview", temperature = 0.7)
  ...
  def __init__(self, name_a, name_b, optional_instructions = ''):
    ...
    self.bot = actor.OneShotLLMActor("Double Crux Facilitator", self.e4_bot, initial_instructions)
    self.step = 'Start'
    self.substep = 0
  ...
  def update_step(self):
    ...
    if (self.substep == len(bot_instructions.get(self.step))):
      ...
      self.step, self.substep = str(discussionFlowAction.conversation_step.value), 0

  def act(self, message = None, expected_type = None):
    for i in range(3): # Arbitrary limit of tries # TODO
      try:
        action_event = self.bot.act(message, expected_type = expected_type)
